...
public class NodeToolCommand extends AbstractCommand {
    ...
    private static ItemStack TOOL_HOTSPOT = ItemBuilder
            .of(Material.BLAZE_ROD)
            .enchant(Enchantment.DURABILITY, 1)
            .flag(ItemFlag.values())
            .name("§cTool: Node (Hotspot)")
            .appendLore("§fCreate a new node by clicking atop a block.")
            .appendLore("§fDestroy an existing node by clicking it.")
            .appendLore("§bA RPGCore tool to create a hotspot node.")
            .build();
    ...
    public void invoke(CommandSender sender, String... args) {
        if (args.length == 3) {
            ...
            if (args[1].equalsIgnoreCase("collectible")) {
                ...
                item = ItemBuilder.of(TOOL_HOTSPOT.clone())
