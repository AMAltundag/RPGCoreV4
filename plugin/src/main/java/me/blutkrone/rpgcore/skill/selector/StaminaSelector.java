...
import me.blutkrone.rpgcore.editor.bundle.selector.EditorStaminaSelector;
...
public class StaminaSelector extends AbstractCoreSelector {
    ...
    public StaminaSelector(EditorStaminaSelector editor) {
    ...
    public List<IOrigin> doSelect(IContext context, List<IOrigin> previous) {
        ...
        for (IOrigin origin : previous) {
            if (origin instanceof CoreEntity) {
                ResourceSnapshot snapshot = ((CoreEntity) origin).getStamina().snapshot();
                if (percentage) {
