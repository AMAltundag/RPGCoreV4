...
public class TSHUploader {
    ...
    public static String upload(File file) throws IOException {
        try {
            ...
            HttpURLConnection connection = (HttpURLConnection) new URL(TSHUploader.URL).openConnection();
            connection.setRequestMethod("PUT");
            connection.setDoOutput(true);
            connection.setRequestProperty("Content-Type", "multipart/form-data; boundary=" + boundary);
            // attempt to push to the upload service
            try (OutputStream output = connection.getOutputStream(); PrintWriter writer = new PrintWriter(new OutputStreamWriter(output, TSHUploader.CHARSET), true)) {
