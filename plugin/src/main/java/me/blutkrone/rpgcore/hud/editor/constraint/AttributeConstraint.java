...
public class AttributeConstraint implements IEditorConstraint {

    @Override
    public List<String> getHint(String value) {
        value = value.toLowerCase();
        List<String> matched = new ArrayList<>();
        for (String translated : RPGCore.inst().getAttributeManager().getIndex().getKeys()) {
    ...
    public boolean isDefined(String value) {
        return RPGCore.inst().getAttributeManager().getIndex().has(value);
    }

    @Override
    public void extend(String value) {
        RPGCore.inst().getAttributeManager().getIndex().get(value);
    }
