...
public class EditorIndex<K, Q extends IEditorRoot<K>> {
    ...
    /**
     * Attempt to create an element with the 'id', also running a
     * pre-register initiator on it. This method is NOT intended
     * for loading!
     *
        ...
        return this.indexed.computeIfAbsent(id, (key -> {
            try {
                ...
                if (config_file.exists()) {
                    ...
                    if (editor == null) {
                        throw new NullPointerException("Editor instance is null!");
                    }
                    editor.setFile(config_file);
                } else {
                    editor = this.editor_factory.get();
